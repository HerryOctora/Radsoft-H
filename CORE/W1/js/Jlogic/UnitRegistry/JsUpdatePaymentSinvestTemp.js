$(document).ready(function () {
    ...
    var GlobValidator = $("#WinUpdatePaymentSInvestTemp").kendoValidator().data("kendoValidator");
    ...
    function initWindow() {
        ...
        $("#BtnImportUpdatePaymentSInvestTemp").kendoButton({
        ...
        //win = $("#WinUpdatePaymentSInvestTemp").kendoWindow({
        //    height: 200,
    ...
    $("#BtnUpdateBySAandFund").click(function () {
        $.ajax({
            url: window.location.origin + "/Radsoft/UpdatePaymentSInvestTemp/GetData_SA/" + sessionStorage.getItem("user") + "/" + sessionStorage.getItem("id"),
            type: 'GET',
        ...
        $.ajax({
            url: window.location.origin + "/Radsoft/UpdatePaymentSInvestTemp/GetData_Fund/" + sessionStorage.getItem("user") + "/" + sessionStorage.getItem("id"),
            type: 'GET',
            contentType: "application/json;charset=utf-8",
            success: function (data) {
                $("#Fund").kendoComboBox({
    ...
    $("#BtnSubmitBySelected").click(function () {
        ...
        var stringUpdatePaymentSInvestTempSelected = '';

        for (var i in ArrayFundFrom) {
            stringUpdatePaymentSInvestTempSelected = stringUpdatePaymentSInvestTempSelected + ArrayFundFrom[i] + ',';

        }
        stringUpdatePaymentSInvestTempSelected = stringUpdatePaymentSInvestTempSelected.substring(0, stringUpdatePaymentSInvestTempSelected.length - 1)

        alert(stringUpdatePaymentSInvestTempSelected);
        if ($("#PaymentDateBySelected").val() == null || $("#PaymentDateBySelected").val() == "") {
            ...
            alertify.confirm("Are you sure want to Change Payment Date ?", function (e) {
                if (e) {
                    ...
                    var UpdatePaymentSInvestTemp = {
                        UpdatePaymentSInvestTempSelected: stringUpdatePaymentSInvestTempSelected,
                    };
                    $.ajax({
                        url: window.location.origin + "/Radsoft/UpdatePaymentSInvestTemp/UpdatePaymentSInvestTemp_ChangePaymentDate_BySelected/" + sessionStorage.getItem("user") + "/" + sessionStorage.getItem("id") + "/" + kendo.toString($("#PaymentDateBySelected").data("kendoDatePicker").value(), "MM-dd-yy") + "/" + kendo.toString($("#FilterDate").data("kendoDatePicker").value(), "MM-dd-yy"),
                        type: 'POST',
                        data: JSON.stringify(UpdatePaymentSInvestTemp),
                        contentType: "application/json;charset=utf-8",
                        success: function (data) {
                            alertify.alert(data);
                            winBySelected.close();
    ...
    $("#BtnSubmitBySAandFund").click(function () {
        ...
        if ($("#PaymentDateBySAandFund").val() == null || $("#PaymentDateBySAandFund").val() == "") {
            ...
            alertify.confirm("Are you sure want to Change Payment Date ?", function (e) {
                if (e) {
                    var UpdatePaymentSInvestTemp_SAandFund = {
                        SA: $('#SA').val(),
                        Fund: $('#Fund').val()
                    };
                    $.ajax({
                        url: window.location.origin + "/Radsoft/UpdatePaymentSInvestTemp/UpdatePaymentSInvestTemp_ChangePaymentDate_BySAandFund/" + sessionStorage.getItem("user") + "/" + sessionStorage.getItem("id") + "/" + kendo.toString($("#PaymentDateBySAandFund").data("kendoDatePicker").value(), "MM-dd-yy") + "/" + kendo.toString($("#FilterDate").data("kendoDatePicker").value(), "MM-dd-yy"),
                        type: 'POST',
                        data: JSON.stringify(UpdatePaymentSInvestTemp_SAandFund),
                        contentType: "application/json;charset=utf-8",
                        success: function (data) {
                            alertify.alert(data);
    ...
    $("#BtnImportUpdatePaymentSInvestTemp").click(function () {
        document.getElementById("FileImportUpdatePaymentSInvestTemp").click();
    });

    $("#FileImportUpdatePaymentSInvestTemp").change(function () {
        if ($("#Transaction").val() == "sub") {
            $.blockUI({});

            var data = new FormData();
            var files = $("#FileImportUpdatePaymentSInvestTemp").get(0).files;
            ...
            if (files.length > 0) {
                data.append("UpdatePaymentSInvestSubRedTemp", files[0]);
                $.ajax({
                    ...
                    success: function (data) {
                        $.unblockUI();
                        $("#FileImportUpdatePaymentSInvestTemp").val("");
                        alertify.alert(data);
                        refresh();
                    },
                    error: function (data) {
            ...
            //var _d = files[0].name.substring(0, 50);
            if (files.length > 0) {
                data.append("UpdatePaymentSInvestTemp", files[0]);
                $.ajax({
                    url: window.location.origin + "/Radsoft/Upload/UploadData/" + sessionStorage.getItem("user") + "/" + sessionStorage.getItem("id") + "/" + "UpdatePaymentSInvestTemp_Import/01-01-1900/0",
                    type: 'POST',
                    data: data,
        ...
        return new kendo.data.DataSource(
             {
                 ...
                 schema: {
                     model: {
                         fields: {
                             UpdatePaymentSInvestTempPK: { type: "number" },
    ...
    function initGrid() {
        ...
        {
            var URL = window.location.origin + "/Radsoft/UpdatePaymentSInvestTemp/GetData/" + sessionStorage.getItem("user") + "/" + sessionStorage.getItem("id") + "/" + kendo.toString($("#FilterDate").data("kendoDatePicker").value(), "MM-dd-yy"),
        ...
        gridData = $("#gridData").kendoGrid({
            ...
            columns: [
                ...
                { field: "UpdatePaymentSInvestTempPK", title: "SysNo.", width: 95 },
    ...
    function refresh() {
        var newDS = getDataSource(window.location.origin + "/Radsoft/UpdatePaymentSInvestTemp/GetData/" + sessionStorage.getItem("user") + "/" + sessionStorage.getItem("id") + "/" + kendo.toString($("#FilterDate").data("kendoDatePicker").value(), "MM-dd-yy"));
    ...
    function selectRowApproved() {
        ...
        checkedIds[dataItemZ.UpdatePaymentSInvestTempPK] = checked;
        if (checked) {
    ...
    function onDataBoundApproved(e) {
        ...
        for (var i = 0; i < view.length; i++) {
            if (checkedIds[view[i].UpdatePaymentSInvestTempPK]) {
    ...
    $("#BtnMoveToSubsRedemp").click(function (e) {
        alertify.confirm("Are you sure want to Move To SubsRedemp ?", function (e) {
            if (e) {
                $.blockUI({});
                $.ajax({
                    url: window.location.origin + "/Radsoft/UpdatePaymentSInvestTemp/UpdatePaymentSInvestTemp_Validate/" + sessionStorage.getItem("user") + "/" + sessionStorage.getItem("id") + "/" + kendo.toString($("#FilterDate").data("kendoDatePicker").value(), "MM-dd-yy"),
                    type: 'GET',
                    contentType: "application/json;charset=utf-8",
                    success: function (data) {
                        if (data.Result == 1) {
                            ...
                            $.ajax({
                                url: window.location.origin + "/Radsoft/UpdatePaymentSInvestTemp/MoveToSubsRedemp/" + sessionStorage.getItem("user") + "/" + sessionStorage.getItem("id") + "/" + kendo.toString($("#FilterDate").data("kendoDatePicker").value(), "MM-dd-yy"),
                                type: 'POST',
                                data: JSON.stringify(UpdatePaymentSInvestTemp),
                                contentType: "application/json;charset=utf-8",
                                success: function (data) {
