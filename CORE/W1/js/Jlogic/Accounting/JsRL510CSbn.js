$(document).ready(function () {
    ...
    function initWindow() {
        ...
        win = $("#WinRL510CSbn").kendoWindow({
            height: 600,
            title: "RL510CSbn Detail",
            visible: false,
            width: 1200,
            modal: true,
            open: function (e) {
    ...
    var GlobValidator = $("#WinRL510CSbn").kendoValidator().data("kendoValidator");
    ...
    function showDetails(e) {
        if (e == null) {
            ...
            e.handled = true;
            var grid;
            grid = $("#gridRL510CSbnApproved").data("kendoGrid");
    ...
    function initGrid() {
        ...
        $("#gridRL510CSbnApproved").empty();
        if ($("#DateFrom").val() == null || $("#DateFrom").val() == "" || $("#DateTo").val() == null || $("#DateTo").val() == "") {
            alertify.alert("Please Fill Date");
        }
        else {

            var RL510CSbnApprovedURL = window.location.origin + "/Radsoft/RL510CSbn/GetDataByDateFromTo/" + sessionStorage.getItem("user") + "/" + sessionStorage.getItem("id") + "/" + 2 + "/" + kendo.toString($("#DateFrom").data("kendoDatePicker").value(), "MM-dd-yy") + "/" + kendo.toString($("#DateTo").data("kendoDatePicker").value(), "MM-dd-yy"),
            dataSourceApproved = getDataSource(RL510CSbnApprovedURL);

        }

        var grid = $("#gridRL510CSbnApproved").kendoGrid({
            ...
            columns: [
                ...
                //{ field: "RL510CSbnPK", title: "SysNo.", width: 95 },
                { field: "Date", title: "Date", width: 200, template: "#= kendo.toString(kendo.parseDate(Date), 'dd/MMM/yyyy')#" },
                { field: "DueDate", hidden: true, title: "Due Date", width: 200, template: "#= kendo.toString(kendo.parseDate(DueDate), 'dd/MMM/yyyy')#" },
